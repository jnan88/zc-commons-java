...
public class KitHash {
	...
	public static byte[] sha256(String input) {
		return digest(input.getBytes(StandardCharsets.UTF_8), get(SHA_256_DIGEST), null, 1);
	}

	/**
	 * 对输入字符串进行sha512散列, 编码默认为UTF8.
	 */
	public static byte[] sha512(String input) {
		return digest(input.getBytes(StandardCharsets.UTF_8), get(SHA_512_DIGEST), null, 1);
	}
	 ...
	 * 对文件进行sha1散列.
	 */
	public static byte[] sha1File(InputStream input) {
	...
	public static byte[] md5(String input) {
		try (InputStream in = new ByteArrayInputStream(input.getBytes(StandardCharsets.UTF_8))) {
			...
			throw new RuntimeException("md5 exception [" + input + ']', e);
		}
	}
	...
	private static byte[] digestFile(InputStream input, MessageDigest messageDigest) {
		...
		try {
			...
			throw new RuntimeException("digestFile exception " + e.getMessage(), e);
		}
