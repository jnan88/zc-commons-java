package xyz.nesting.core.concurrent.threadpool;
...
import xyz.nesting.core.concurrent.threadpool.QueuableCachedThreadPool.ControllableQueue;
 ...
 */
public class KitThreadPoolBuilder {
	...
	private static ThreadFactory createThreadFactory(ThreadFactory threadFactory, String threadNamePrefix,
		...
		if (threadNamePrefix != null) {
			if (daemon != null) {
				return KitThreadPoolUtil.buildThreadFactory(threadNamePrefix, daemon);
			} else {
				return KitThreadPoolUtil.buildThreadFactory(threadNamePrefix);
			}
		}
